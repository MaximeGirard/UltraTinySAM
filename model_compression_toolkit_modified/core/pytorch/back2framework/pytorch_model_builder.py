...
def _run_operation(n: BaseNode,
    ...
    if isinstance(n, FunctionalNode) and n.inputs_as_list:
        if isinstance(op_func, PytorchQuantizationWrapper):
            ...
            # print("Running op_func", op_func)
            # print(("Input tensors shapes", [i.shape for i in input_tensors]))
        ...
        if isinstance(op_func, PytorchQuantizationWrapper) and isinstance(n, FunctionalNode) and n.functional_op is not torch.gather:
            ...
            # print("Node name", n.name)
            # print("Running op_func", op_func)
            # print("Merged inputs types", [
            #     [elem.shape if isinstance(elem, torch.Tensor) else elem for elem in i] if isinstance(i, tuple)
            #     else i.shape if isinstance(i, torch.Tensor)
            #     else i
