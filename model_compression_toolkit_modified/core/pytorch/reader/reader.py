...
def remove_broken_nodes_from_graph(graph):
    ...
    """
    # Print all nodes and their output
    for node in graph.nodes:
        print(f"Node: {node}, Output n: {graph.out_degree(node)}, input nodes: {graph.get_prev_nodes(node)}")
    ...
    print(f"nodes_list without output: {nodes_list}")
    nodes_to_remove = []
        ...
        while len(nodes_to_remove) != 0:
            for node_to_remove in nodes_to_remove:
                ...
                for parent_node in parent_nodes:
                    # I add this patch because I (strangely) get a node with no output but with an edge (after) to the output node
                    if parent_node not in output_nodes:
                        ...
                        if graph.out_degree(parent_node) == 1:
                            ...
                            print(f"Node {node_to_remove} parent to remove : Node: {parent_node}, Output n: {graph.out_degree(parent_node)}, input nodes: {graph.get_prev_nodes(parent_node)}")
