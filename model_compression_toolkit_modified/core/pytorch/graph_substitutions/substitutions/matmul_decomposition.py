...
class MatMulParams:
    ...
    def __init__(self,
        ...
        print("other_shape", self.other_shape)
        ...
        print("input_reshape_shape", self.input_reshape_shape)
        print("other_reshape_shape", self.other_reshape_shape)
        ...
        print("single_matmul_shape", self.single_matmul_shape)
        ...
        print("matmul_stack_shape", self.matmul_stack_shape)
...
class MatMulDecomposition(BaseSubstitution):
    ...
    def substitute(self,
        ...
        # I patch the fact that sometimes matmul has not a shape (a, b, c, d) but ((a, b, c, d,),)
        # This is a bug in the graph creation, i don't know why
        input_shape_copy_list = list(matmul_node.input_shape)
        print("input_shape_copy_list", input_shape_copy_list)
        for i in range(len(input_shape_copy_list)):
            if isinstance(input_shape_copy_list[i][0], list):
                    input_shape_copy_list[i] = input_shape_copy_list[i][0]
        print("input_shape_copy_list after processing", input_shape_copy_list)
        matmul_node.input_shape = input_shape_copy_list
        
        # If both matrices are already 3D or less, no need to change the graph
        if len(input_shape_copy_list[0]) <= 3 and len(input_shape_copy_list[1]) <= 3:
            return graph
        ...
        if len(matmul_node.input_shape[0]) != len(matmul_node.input_shape[1]):
            Logger.critical(f'Mismatch between number of input dimensions in node {matmul_node.name}. '
                             f'Input shapes: {matmul_node.input_shape[0]} and {matmul_node.input_shape[1]}')

        print("Found matmul node with input shape", matmul_node.input_shape)
        matmul_params = MatMulParams(matmul_node)


        # Expand inputs to equal dimensions (instead of broadcasting) - if needed
        if not np.array_equal(matmul_node.input_shape[1:-2], matmul_params.other_shape[1:-2]):
        ...
        # (B, D_1, ... , D_N, m, p) --> (B, (D_1*...*D_N), m, p)
        # (B, D_1, ... , D_N, p, n) --> (B, (D_1*...*D_N), p, n)
        if len(matmul_node.input_shape) > 4:  # both input & other have the same number of dimensions
        ...
        print("Matmul node", matmul_node.name, "substituted with", len(split_matmul_nodes), "matmuls")
