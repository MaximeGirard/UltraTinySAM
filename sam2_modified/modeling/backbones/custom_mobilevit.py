...
class MobileViTSelfAttention(nn.Module):
    def __init__(self, config: MobileViTConfig, hidden_size: int, exportable: bool = False) -> None:
        super().__init__()
    ...
    def transpose_for_scores(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.exportable:
            # transpose_for_scores x shape: torch.Size([1, 4, 4096, 96])
            
            new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
            # transpose_for_scores new_x_shape: torch.Size([1, 4, 4096, 4, 24])
            
            x = x.view(new_x_shape[0], new_x_shape[1], new_x_shape[2], new_x_shape[3], new_x_shape[4])
        
            return x.permute(0, 1, 3, 2, 4)

        else:           
            # transpose_for_scores x shape: torch.Size([4, 4096, 96])
            
            new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
            # transpose_for_scores new_x_shape: torch.Size([4, 4096, 4, 24])
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # flat the 1 and 2 dim
        if self.exportable:
            B, Np, N, H, W = query_layer.shape
            query_layer = query_layer.reshape(B, Np * N, H, W)
            key_layer = key_layer.reshape(B, Np * N, H, W)
            value_layer = value_layer.reshape(B, Np * N, H, W)
        ...
        context_layer = F.scaled_dot_product_attention(
            query_layer,
            key_layer, # ?? => problem between pytorch and mct
            value_layer,
        ...
        if self.exportable:
            ...
            context_layer = context_layer.reshape(B, Np, N, H, W)

            context_layer = context_layer.permute(0, 1, 3, 2, 4).contiguous()
            
            new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
            context_layer = context_layer.view(new_context_layer_shape[0], new_context_layer_shape[1], new_context_layer_shape[2], new_context_layer_shape[3])
...
class MobileViTAttention(nn.Module):
    def __init__(self, config: MobileViTConfig, hidden_size: int, exportable: bool = False) -> None:
        super().__init__()
        self.attention = MobileViTSelfAttention(config, hidden_size, exportable=exportable)
...
class MobileViTTransformerLayer(nn.Module):
    def __init__(self, config: MobileViTConfig, hidden_size: int, intermediate_size: int, exportable: bool = False) -> None:
        super().__init__()
        self.attention = MobileViTAttention(config, hidden_size, exportable=exportable)
...
class MobileViTTransformer(nn.Module):
    def __init__(self, config: MobileViTConfig, hidden_size: int, num_stages: int, exportable: bool = False) -> None:
        super().__init__()
...
class MobileViTLayer(nn.Module):
    ...
    def unfolding_exportable(self, features: torch.Tensor) -> Tuple[torch.Tensor, Dict]:       
        patch_width, patch_height = self.patch_width, self.patch_height
        patch_area = int(patch_width * patch_height)

        batch_size, channels, orig_height, orig_width = features.shape
        
        # In exportable mode, we need to ensure that the batch size is 1
        # assert batch_size == 1, "Batch size must be 1 for exportable unfolding."

        interpolate = False

        # number of patches along width and height
        num_patch_width = orig_width // patch_width
        num_patch_height = orig_height // patch_height
        num_patches = num_patch_height * num_patch_width

        # convert from shape (batch_size (=1), channels, orig_height, orig_width)
        # to the shape ( batch_size (=1), patch_area, num_patches, channels)
        
        patches = features.reshape(
            batch_size, channels * num_patch_height, patch_height, num_patch_width, patch_width
        )
        
        # (batch_size (=1), channels * num_patch_height, patch_height, num_patch_width, patch_width)
        
        patches = patches.transpose(2, 3)
        
        # (batch_size (=1), channels * num_patch_height, num_patch_width, patch_height, patch_width)
        
        patches = patches.reshape(batch_size, channels, num_patches, patch_area)
        
        # (batch_size (=1), channels, num_patches, patch_area)
        
        patches = patches.transpose(1, 3)
        
        # (batch_size (=1), patch_area, num_patches, channels)
    ...
    def folding_exportable(self, patches: torch.Tensor, info_dict: Dict) -> torch.Tensor:
        patch_width, patch_height = self.patch_width, self.patch_height
        patch_area = int(patch_width * patch_height)
        ...
        # assert batch_size == 1, "Batch size must be 1 for exportable folding."
        # In exportable mode, we need to ensure that the batch size is 1

        # convert from shape (batch_size (=1), patch_area, num_patches, channels)
        # back to shape (batch_size, channels, orig_height, orig_width)
        ...
        # (batch_size (=1), channels, orig_height, orig_width)
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        if self.exportable:
            patches, info_dict = self.unfolding_exportable(features)
        else:
            patches, info_dict = self.unfolding(features)

        # learn global representations
        # Here problem : must explicitly process patch by patch
        patches = self.transformer(patches)
        patches = self.layernorm(patches)

        # convert patches back to feature maps
        if self.exportable:
            features = self.folding_exportable(patches, info_dict)
        else:
...
class MobileViTEncoder(nn.Module):
    def __init__(self, config: MobileViTConfig, exportable: bool = False) -> None:
        super().__init__()
        self.config = config
        self.exportable = exportable
...
class MobileViTModel(MobileViTPreTrainedModel):
    def __init__(self, config: MobileViTConfig, expand_output: bool = True, exportable: bool = False) -> None:
        ...
        self.conv_stem = MobileViTConvLayer(
            ...
            kernel_size=3,
            stride=2, # Here! try stride = 1 ?? to allow 1 more layer in the encoder
        )

        self.encoder = MobileViTEncoder(config, exportable=exportable)

        if self.expand_output:
            self.conv_1x1_exp = MobileViTConvLayer(
                config,
                in_channels=config.neck_hidden_sizes[4], # this is modified
