...
class VOSDataset(VisionDataset):
    ...
    def _get_datapoint(self, idx):
        ...
        # print("Loading video: ", video.video_id)
        # print("Frames: ", len(datapoint.frames))
        # print("Data: ", datapoint.frames[0].data.shape)
        # print("Number of objects: ", len(datapoint.frames[0].objects))
        # print("Objects: ", datapoint.frames[0].objects[0].segment.shape)
        
        # Save the image
        # img = datapoint.frames[0].data.permute(1, 2, 0).cpu().numpy()
        ...
        # # Save the masks
        # for j, obj in enumerate(datapoint.frames[0].objects):
        #     mask = obj.segment.cpu().numpy()
        #     mask = (mask * 255).astype(np.uint8)
        #     cv2.imwrite(f"mask_0_{j}.png", mask)

        # print(f"Saved img and masks in {os.getcwd()}")
