...
def load_selected_state_dict(
    ...
    """
    Loads selected parts of a state dict into the given model.

    Args:
        state_dict: A dictionary containing the model's state dict.
        model: Model to load the checkpoint weights into.
        selected_keys: List of keys (or key prefixes) to load.
        strict: Whether to enforce that all state_dict keys match model keys.
        ignore_missing_keys: List of keys to ignore if missing.
        ignore_unexpected_keys: List of keys to ignore if unexpected.
        checkpoint_kernels: Functions to process state_dict before loading.
    """
    # Apply any transformation kernels
    if checkpoint_kernels is not None:
        for f in checkpoint_kernels:
            state_dict = f(state_dict=state_dict)
    
    # If selected_keys is specified, filter state_dict to include only those keys
    if selected_keys is not None:
        state_dict = {k: v for k, v in state_dict.items() if any(k.startswith(sk) for sk in selected_keys)}
    
    print(f"Loading keys: {list(state_dict.keys())}")
    
    # Load filtered state_dict into model
    missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
