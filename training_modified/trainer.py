...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        checkpoint_teacher: Optional[Dict[str, Any]] = None,
        max_epochs: int,
        mode: str = "train",
        ...
        if checkpoint_teacher is not None:
            self.checkpoint_teacher_conf = CheckpointConf(
                **checkpoint_teacher
            ).infer_missing()
        else:
            self.checkpoint_teacher_conf = None
        ...
        if self.teacher_model_conf is not None and compute_teacher:
            self.load_checkpoint_teacher()
        ...
        num_params = sum(p.numel() for p in self.model.image_encoder.parameters())
        print(f"Number of trainable parameters in image encoder: {num_params}")
        # Print the total number of parameters in the decoder
        num_params = sum(p.numel() for p in self.model.sam_mask_decoder.parameters())
        ...
        num_params = sum(p.numel() for p in self.model.sam_prompt_encoder.parameters())
        ...
        if (
            hasattr(self.model, "memory_encoder")
            and self.model.memory_encoder is not None
        ):
            num_params = sum(p.numel() for p in self.model.memory_encoder.parameters())
        ...
        if (
            hasattr(self.model, "memory_attention")
            and self.model.memory_attention is not None
        ):
            num_params = sum(
                p.numel() for p in self.model.memory_attention.parameters()
            )
        ...
        for name, param in self.model.named_parameters():
            if not param.requires_grad:
                print(f"Frozen layer: {name} | Number of parameters: {param.numel()}")
    ...
    def load_checkpoint_teacher(self):
        self._init_teacher_model_state()
    ...
    def _init_teacher_model_state(self):
        # Checking that parameters that won't be saved are indeed frozen
        # We do this check here before even saving the model to catch errors
        # are early as possible and not at the end of the first epoch
        assert_skipped_parameters_are_frozen(
            patterns=self.checkpoint_teacher_conf.skip_saving_parameters,
            model=self.model,
        )
        ...
        allow_init_skip_parameters = (
            self.checkpoint_teacher_conf.initialize_after_preemption
        )
        with with_check_parameter_frozen(
            patterns=self.checkpoint_teacher_conf.skip_saving_parameters,
            model=self.model,
            ...
            self._call_teacher_model_initializer()
    ...
    def _call_teacher_model_initializer(self):
        model_weight_initializer = instantiate(
            self.checkpoint_teacher_conf.model_weight_initializer
        )
        if model_weight_initializer is not None:
            logging.info(
                f"Loading teacher pretrained checkpoint from {self.checkpoint_teacher_conf.model_weight_initializer}"
            )
            self.teacher_model = model_weight_initializer(model=self.teacher_model)
    ...
    def _load_resuming_checkpoint_teacher(self, ckpt_path: str):
        logging.info(f"Loading teacher from {ckpt_path}")
        ...
        load_state_dict_into_model(
            ...
            state_dict=checkpoint["model"],
            ignore_missing_keys=self.checkpoint_teacher_conf.skip_saving_parameters,
        )
    ...
    def _step(
        ...
        save_dir = os.path.join(self.save_dir, phase, str(self.epoch))

        outputs, intermediate_0, intermediate_1 = model(batch)
        targets = batch.masks
        if teacher_model is not None and self.compute_teacher:
            teacher_outputs, teacher_intermediate_0, teacher_intermediate_1 = teacher_model(batch)
        else:
            teacher_outputs, teacher_intermediate_0, teacher_intermediate_1 = None, None, None

        self._save_visualizations(batch, outputs, teacher_outputs, save_dir)
        ...
        loss = self.loss[key](
            ...
            targets,
            teacher_outputs,
            [intermediate_0, intermediate_1],
            [teacher_intermediate_0, teacher_intermediate_1]
        )
        ...
        if isinstance(loss, dict):
            step_losses = {f"Losses/{phase}_{key}_{k}": v for k, v in loss.items()}
                ...
                with torch.cuda.amp.autocast(
                    ...
                    for phase, model in zip(curr_phases, curr_models):
                        ...
                        for k, v in extra_losses.items():
                            ...
                            extra_loss_mts[k].update(value, batch_size)
    ...
    def _run_step(
        ...
        for extra_loss_key, extra_loss in extra_losses.items():
            ...
            extra_loss_mts[extra_loss_key].update(value, batch_size)
    ...
    def _setup_components(self):
        ...
        # Commenting for now bc doesn't work with our config ??
    ...
    def _save_visualizations(self, batch, outputs, teacher_outputs, save_dir):
        ...
        try:
            frame_per_video = batch.img_batch.shape[1]
            preds = outputs[0]["pred_masks_high_res"]  # shape: [num_objects, 1, H, W]
            masks = batch.masks  # shape: [1, num_objects, H, W]
            obj_to_frame_idxs = batch.obj_to_frame_idx[
                0
            ]  # shape: [num_objects, 2] -> (video_idx, frame_idx)
            image_size = self.model_conf.image_size

            num_objects = preds.shape[0]
            mask_colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]

            # Group mask indices by frame
            frame_to_mask_idxs = {i: [] for i in range(frame_per_video)}
            for idx, (_, frame_idx) in enumerate(obj_to_frame_idxs.tolist()):
                frame_to_mask_idxs[frame_idx].append(idx)

            for i in range(frame_per_video):
                img = self._tensor_to_image(batch.img_batch[0][i])
                img_with_masks = img.copy()
                img_with_preds = img.copy()
                img_with_teacher_preds = (
                    img.copy() if teacher_outputs is not None else None
                )

                if teacher_outputs is not None:
                    teacher_preds = teacher_outputs[0]["pred_masks_high_res"]

                point_coords, point_labels = self._extract_points(
                    outputs[0]["point_inputs"], i, obj_to_frame_idxs
                )

                mask_indices = frame_to_mask_idxs.get(i, [])
                for j, obj_idx in enumerate(mask_indices):
                    color = mask_colors[j % len(mask_colors)]  # cycle if >3 masks

                    gt_mask = (masks[0][obj_idx].cpu().numpy() * 255).astype(np.uint8)
                    pred_mask = (preds[obj_idx][0].detach().cpu().numpy() > 0.5).astype(
                        np.uint8
                    )

                    if teacher_outputs is not None:
                        teacher_pred_mask = (
                            teacher_preds[obj_idx][0].detach().cpu().numpy() > 0.5
                        ).astype(np.uint8)

                    for c in range(3):
                        img_with_masks[:, :, c] = np.where(
                            gt_mask > 0, color[c], img_with_masks[:, :, c]
                        )
                        img_with_preds[:, :, c] = np.where(
                            pred_mask > 0, color[c], img_with_preds[:, :, c]
                        )
                        if teacher_outputs is not None:
                            img_with_teacher_preds[:, :, c] = np.where(
                                teacher_pred_mask > 0,
                                color[c],
                                img_with_teacher_preds[:, :, c],
                            )

                    self._draw_points_or_boxes(
                        img_with_masks, point_coords[j] if j < len(point_coords) else []
                    )
                    self._draw_points_or_boxes(
                        img_with_preds, point_coords[j] if j < len(point_coords) else []
                    )
                    if teacher_outputs is not None:
                        self._draw_points_or_boxes(
                            img_with_teacher_preds,
                            point_coords[j] if j < len(point_coords) else [],
                        )

                cv2.imwrite(f"{save_dir}/img_{i}_with_masks.png", img_with_masks)
                cv2.imwrite(f"{save_dir}/img_{i}_with_preds.png", img_with_preds)
                if teacher_outputs is not None:
                    cv2.imwrite(
                        f"{save_dir}/img_{i}_with_teacher_preds.png",
                        img_with_teacher_preds,
                    )

            print(f"Saved visualizations to {os.path.abspath(save_dir)}")

        except Exception as e:
            print(f"Error in visualization: {e}")
            print("Skipping visualization for this batch.")
    ...
    def _extract_points(self, point_inputs, frame_idx, obj_to_frame_idx):
        point_coords = point_inputs["point_coords"]  # [num_objects, num_points, 2]
        point_labels = point_inputs["point_labels"]  # [num_objects, num_points]

        # Get indices of objects that belong to this frame
        obj_indices = (obj_to_frame_idx[:, 1] == frame_idx).nonzero(as_tuple=True)[0]

        # Select only relevant points
        frame_points = point_coords[obj_indices]  # [num_objs_in_frame, num_points, 2]
        frame_labels = point_labels[obj_indices]  # [num_objs_in_frame, num_points]

        return frame_points.cpu().numpy(), frame_labels.cpu().numpy()

    def _draw_points_or_boxes(self, img, points):
        if points.shape[0] == 1:  # Single points
            x, y = points[0]
            cv2.drawMarker(
                img,
                (int(x), int(y)),
