...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        modules_to_freeze=[],
        loss_with_intermediate_outputs=False,
        **kwargs,
    ):
        ...
        self.loss_with_intermediate_outputs = loss_with_intermediate_outputs
        ...
        for name, param in self.named_parameters():
            if any(name.startswith(module) for module in modules_to_freeze):
                param.requires_grad = False
                print(f"Froze: {name}")  # Debugging: Check what gets frozen
    ...
    def forward(self, input: BatchedVideoDatapoint):
        if self.training or not self.forward_backbone_per_frame_for_eval:
            ...
            if self.return_intermediate_embeddings:
                backbone_out, intermediate_0, intermediate_1 = self.forward_image(input.flat_img_batch)
                # print("[BACKBONE OUT]", intermediate_0.shape, intermediate_1.shape)
            else:
        ...
        if self.return_intermediate_embeddings:
            return previous_stages_out, intermediate_0, intermediate_1
        else:
    ...
    def _prepare_backbone_features_per_frame(self, img_batch, img_ids):
        ...
        if self.return_intermediate_embeddings:
            backbone_out, intermediate_0, intermediate_1 = self.forward_image(image)
            # print("[BACKBONE OUT]", intermediate_0.shape, intermediate_1.shape)
        else:
